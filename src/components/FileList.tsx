...
export const FileList = ({forUpload}:FileListProps) => {
  ...
  return (<div className={"content-container divide-y-2"}>
    {files?.map((f, idx) => (
    ...
    <div className={"container flex transition duration-50 hover:bg-slate-100 justify-between"}>
      <label htmlFor={forUpload} className={"w-full cursor-pointer p-2 text-secondary"}>Add {files?.length > 0 ? "more" : "files"}...</label>
    </div>
  </div>);
}